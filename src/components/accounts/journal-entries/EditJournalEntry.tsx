...
export default function EditJournalEntry() {
  ...
  useEffect(() => {
    const loadJournalEntry = () => {
      ...
      try {
        ...
        const entry = existingEntries.find((e: any) => e.id === id);
        ...
        // Convert lines to the format expected by the form
        const formattedLines = entry.lines.map((line: any) => ({
          account: getAllAccounts().find(acc => acc.name === line.account)?.code || "",
          debit: line.debit || 0,
          credit: line.credit || 0
        }));
        
        setLines(formattedLines.length > 0 ? formattedLines : [
          { account: "", debit: 0, credit: 0 },
          { account: "", debit: 0, credit: 0 },
        ]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ 
        ...
        description: `Journal entry ${id} has been updated successfully.` 
      });
      navigate(`/accounts/journal-entries/${id}`);
    } catch (error) {
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" onClick={() => navigate(`/accounts/journal-entries/${id}`)}>
          <ArrowLeft className="h-4 w-4" />
        </Button>
        <div>
          <h1 className="text-3xl font-bold">Edit Journal Entry - {id}</h1>
          <p className="text-muted-foreground">Update journal entry details</p>
        </div>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex justify-end gap-4">
              <Button type="button" variant="outline" onClick={() => navigate(`/accounts/journal-entries/${id}`)}>Cancel</Button>
